...
if ("Update Schedule"==$Action || "Clear Schedule"==$Action2){ //Are we updating the schedule?
...
$warnings = '';
if($AvailableHrs < 30) {
	$warnings .= 'You need at least 30 hours of availability,<br />';
}

if($warnings != '') {
?>
	<BR /><FONT COLOR="FF0000">Warnings:
<?
	echo $warnings, '</FONT>';
}
?>
...
<INPUT TYPE=HIDDEN NAME="IdNum" VALUE=<? echo $IdNum; ?>>
<INPUT TYPE=HIDDEN NAME="SID" VALUE=<?echo $SID;?>>
<TABLE ALIGN="left" border=1><TR><TD>Time</TD><TD>Su</TD><TD>M</TD><TD>Tu</TD><TD>W</TD><TD>Th</TD><TD>F</TD><TD>Sa</TD><?=((1==$Holiday)?"<TD>Hol</TD>":"")?></TR>
...
for ($thetime=$Earliest; $thetime<=$Latest; $thetime+=$SmallestBreak) {
	...
	$hr24start=((int)((($thetime)/4)+5)%24);
	$hr24end=((int)((($thetime+$SmallestBreak)/4)+5)%24);
	...
	for ($theday=0;$theday<(7+$Holiday);$theday++) {
		...
		//Check for times where helpdesk is closed
		$isClosed = 0;
		/*
		Disabling checkboxes where we are not open has been deleted due to confusion during finals week -JT
		if($theday == 1 || $theday == 2 || $theday == 3 || $theday == 4 || $theday == 5) //If M-F
		{
			if($realhr < 7.5) //If before 7:30
				$isClosed = 1;
		}
		if($theday == 5 || $theday == 6) //If Friday or Saturday
		{
			if($realhr >= 22 || $realhr <= 2)
				$isClosed = 1;
		}
		if($theday == 0 || $theday == 6) //If Sunday or Saturday
		{
			if($realhr < 9)
				$isClosed = 1;
		}
...
//echo $pressed; 
		...
		<INPUT NAME="T<?=($thetime*8+$theday)?>" TYPE=CHECKBOX CLASS="scheduleBox" VALUE="<?=$pressed?>" <?=(("Y"==$MyOK[$thetime][$theday])?"CHECKED":"")?> <?=(($isClosed==1)?"DISABLED":"")?>>
		</TD><?
	}
...
<TABLE ALIGN="right"> 
<TR>
	<TD><button TYPE="submit" NAME='Action' VALUE="Update Schedule">Update Schedule</button></TD>
	<TD><button  NAME='Action' ONCLICK="clearSchedule();" VALUE="Clear Schedule">Clear Hours</button></TD> 
</TABLE>
</DIV>
</FORM>
<?php
/* if ("Clear Schedule" == $Action) 
{
for ($thetime=$Earliest; $thetime<=$Latest; $thetime+=$SmallestBreak) {
	//Loop through the days of the week
	for ($theday=0;$theday<(7+$Holiday);$theday++) {
		...
		<INPUT NAME="T<?=($thetime*8+$theday)?>" TYPE=CHECKBOX VALUE="<?=$pressed?>" <?=("")?> <?=(($isClosed==1)?"DISABLED":"")?>>
		</TD><?
	}
