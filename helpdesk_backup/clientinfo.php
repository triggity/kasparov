...
if (0==mysql_num_rows($myclient)||0==$CID) {
	...
	if (""!=$MAC) { //Allow only hex numbers in mac
		$MAC=eregi_replace("[^0-F]","",$MAC);
	}
	if (""!=$Room) { //Allow only number in room number
		$Room=eregi_replace("[^0-9]","",$Room);
	}

	if (""!=$Jack) { //Allow only number in jack number
		$Jack=eregi_replace("[^0-9]","",$Jack);
	}

    //Do we need to perform any update?

    if (""!=$Action && (0==$IsUser || "Open Ticket"==$Action || "Quick Ticket"==substr($Action,0,12) || "Add Computer"==$Action || "Add NIC"==$Action)) {
	...
	if (ereg("^Remove",$Action) && (!ereg("Cancel",$Confirmation))) { //Removals
		if ($Action!=$Confirmation) { //Not yet appoved for removal
			...
			$IText = mysql_result($InfoWhat,0,1);

			//Remove it
			$x=mysql_query("DELETE FROM MiscInfo WHERE CampusID=$CID AND ID=$InfoID");
		}
	}
	...
	if (("Correct Info"==$Action) && (0==$IsUser)) {
	...
	} else if (("Open Ticket"==$Action || "Quick Ticket"==substr($Action,0,12)) && ""!=$Description) {
		...
		$x=mysql_query("INSERT INTO PaperTrail (Creator_CampusID, Comment,Department, TicketID,IsFirst,IsLast,Creation,State) VALUES ($CampusID,'".$Description."','".(($userdata["IsLINC"]=="Y")?"LINC":"IT")."',$InsertID,'Y','Y',$CreateDate,'".((substr($Action,0,12)=="Quick Ticket")?"CLOSED":"OPEN")."')");
	...
	?><TABLE border=0 cellspacing=0 cellpadding=3 width=100% BGCOLOR=<?=$color_table_lt_bg?>><TR><TD valign=top BGCOLOR=<?=$color_table_lt_bg?>><BR><H1><?
	echo "&nbsp;".$about["First"]." ".$about["Middle"]." ".$about["Last"];
	?></H1></TD><TD valign=top Align=right BGCOLOR=<?=$color_table_lt_bg?>><TABLE border=0><TR><TD bgcolor=<?=$color_table_dk_bg?>><BR><H1>&nbsp;<?
	echo $about["CampusID"];
	...
	<TR><TD BGCOLOR=<?=$color_table_title?>><?=$ofont_title?>Corrections in Basic Client Information<?=$cfont_title?></TD></TR>
	<TR><TD BGCOLOR=<?=$color_table_lt_bg?> valign=top align=center>
		...
		<? if ($userdata["IsAdmin"]=="Y") {
			?></TR><?
			echo "<TD colspan=5><A HREF=\x22myinfo.php?SID=$SID&IdNum=$CID\x22>Edit CampusID, Password, Schedule, etc...</A></TD>";
		} ?>
		</TR>
		</TABLE>
	</TD></TR>
   ...
   <? } ?>
	...
	if (0==$IsUser) {
	?><TR><TD colspan=3 BGCOLOR=#<? echo ($color==1)?$color_table_dk_bg:$color_table_lt_bg ?>>
...
<?	//data from blueform server
	if (0==$IsUser) {
		?><BR><BR><table border=0 cellspacing=2 cellpadding=2 width=100%>
<tr><td COLSPAN=3 BGCOLOR=<?=$color_table_title?>><?=$ofont_title?>Blueform Data<?=$cfont_title?></td></tr><tr><td colspan=3 BGCOLOR=<?=$color_table_dk_bg?>>
	<TABLE border=0><TR><TD valign=top>
		<B>Client Info:</B><BR><BR>

<?

		
$bfdata=mysql_connect($forms_url,$forms_uname,$forms_upass);

		mysql_select_db($forms_name, $bfdata);
		...
		if (mysql_num_rows($Person)>0) {
			...
			echo "<TR><TD>Full Name:&nbsp;</TD><TD>".mysql_result($Person,0,"FullName")."<BR></TD><TR>";
			echo "<TR><TD>Phone:</TD><TD>".mysql_result($Person,0,"Phone")."<BR></TD><TR>";
			echo "<TR><TD>E-Mail:</TD><TD>".mysql_result($Person,0,"EMail")."@scu.edu<BR></TD><TR>";
			?></TABLE><?
		...
		$Ports=mysql_query("
			...
			FROM
				MACtoCID, MacToPort,PortToRoom,Subnets,Switches, Computers
			WHERE
				MACtoCID.CampusID=$CID AND
				MacToPort.MAC=MACtoCID.MAC AND
				PortToRoom.Switch=MacToPort.Switch AND
				PortToRoom.Module=MacToPort.Module AND
				PortToRoom.Port=MacToPort.Port AND
				Switches.Switch=PortToRoom.Switch AND
				Subnets.Start=Switches.Building AND
				Computers.MAC=MACtoCID.MAC
		");
		?><B>Computer(s):</B><BR><BR><?
		//MakeTable($Ports,1,1,1,1,"x");
		MakeTableSideways($Ports,"");
		mysql_close($bfdata);
...
?>
	...
	<OPTION>StaffInfo-Birthday</OPTION>
	...
	<BR><BR><?
	$ComputerList=mysql_query("SELECT ComputerID AS Hidden,Brand, concat('queries.php?QID=3&notables=1&CBrand-Opts=I&CBrand=',Brand) AS 'BrandPopup', Line, Model, OS, OSVer".((($IsUser==0 || $userdata["IsHelpDesk"]=="Y" || $userdata["IsFieldSupport"]=="Y") && 1!=$OnlyInfo )?",'update' AS 'Options', concat('clientinfo.php?SID=$SID&CID=$CID&CompID=',ComputerID,'&Brand=',REPLACE(Brand,' ','+'),'&Line=',REPLACE(Line,' ','+'),'&Model=',REPLACE(Model,' ','+'),'&OS=',REPLACE(REPLACE(OS,' ','+'),'/','%2F'),'&OSVer=',REPLACE(OSVer,' ','+'),'&UPGRADECOMP=YES&notables=$notables&sideless=$sideless') AS OptHyperlink":"")." FROM Computer WHERE Computer.CampusID=$CID", $hdb);
...
if ((0==$IsUser || $userdata["IsHelpDesk"]=="Y" || $userdata["IsFieldSupport"]=="Y")&& 1!=$OnlyInfo) {
	...
	?><TR><TD colspan=6 BGCOLOR=#<? echo ($color==1)?$color_table_dk_bg:$color_table_lt_bg ?>>
...
if (($userdata["IsHelpDesk"]=="Y" || $userdata["IsFieldSupport"]=="Y" || $userdata["IsAdmin"]=="Y")& 1!=$OnlyInfo) {
	...
	<TR>
		<TD rowspan=2><TEXTAREA NAME="Description" COLS=32 ROWS=5></TEXTAREA>&nbsp;</TD>
		...
		<TD valign=top>
			...
			<INPUT TYPE=SUBMIT NAME="Action" VALUE="Quick Ticket*"><BR><BR>
			<INPUT TYPE=SUBMIT NAME="Action" VALUE="Open Ticket">
		</TD>
		<TR><TD colspan=5>
		* Quick Ticket is a feature with creates a new call ticket immediately in a "closed" state. This is for use by helpdesk solving problems over the phone. "Open Ticket" is to add an open call ticket, to be picked up by an RCC.
		</TD></TR>
	</TR>
	</TABLE>
	</TD></TR></TABLE><?
	...
	echo "Oh shit...something really bad happened. Report this immediately";
}
