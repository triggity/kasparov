...
if (2>$onthis || 1==$printversion) {$noedit=1;} else {$noedit=0;}
...
<INPUT TYPE=HIDDEN NAME="Schedule" VALUE="<?=$Schedule?>">
	...
	for ($i=0;$i<mysql_num_rows($timedata);$i++) {
		...
		if (mysql_result($timedata,$i,2)==$Schedule) { //add only if they're working this schedule
			$TheSchedule[$cid][-1]++;
			$TheSchedule[$cid][(int)(mysql_result($timedata,$i,1)/$thisquantum)]++;
					...
					if ($Updateing) {
						if (1==$ischecked && 1!=$GLOBALS['C'.$cid.'T'.$i]) {
							...
							for ($hr=0;$hr<$thisquantum;$hr++) {
								...
								$ClearDelta.= "(CampusID=".$cid." AND Time=".($i*$thisquantum+$hr).")";
							}
							...
							for ($hr=0;$hr<$thisquantum;$hr++) {
								...
								$SetDelta.= "(CampusID=".$cid." AND Time=".($i*$thisquantum+$hr).")";
							}
	...
	if ($noedit==0) {
		//Update the schedule
		if (0<$ClearDeltaCount) mysql_query("UPDATE Schedule_Data SET Schedule=\x22-1\x22 WHERE Day=".$day." AND (".$ClearDelta.")");
		if (0<$SetDeltaCount) mysql_query("UPDATE Schedule_Data SET Schedule=".$Schedule." WHERE Day=".$day." AND (".$SetDelta.")");
	...
	<TR><TD>Availability</TD><?
		...
		for ($i=(floor(((floor($thisstart/4))*4)/$thisquantum));$i<$howfar;$i++) {
			?><TD colspan=<?=$thisquantum?> BGCOLOR=#<?
			if ($thisquantum<=$PeopleScheduled[$i]) {
				?>00FF00<?
			} else {
