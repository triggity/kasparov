...
#######################################################################
*/
function html_graph($names, $values, $bars, $vals, $dvalues=0, $dbars=0, $evalues=0,$ebars=0) 
   {
    ...
    elseif ($vals["type"] == 4)
       {
        triple_vertical_graph($names, $values, $bars, $vals, $dvalues, $dbars, $evalues, $ebars);
       }
    ...
    if (($vals["vlabel"]) || ($vals["hlabel"]))
       {
        ...
        elseif ($vals["type"] == 1 || ($vals["type"] == 3 )|| ($vals["type"] == 4 )) // vertical chart
           {
            $rowspan = 3;
...
?>
         ...
         <IMG SRC="<? echo $bars[$i] ?>" WIDTH=7 HEIGHT="<? if ($values[$i]!=0){
		echo $values[$i] * $vals["scale"];
		} else { echo "1";} ?>">
...
?>
         ...
         <IMG SRC="<? echo $dbars[$i] ?>" WIDTH=7 HEIGHT="<? if ($dvalues[$i]!=0){
...
####################################################################### 
#
#  Function:  triple_vertical_graph($names, $values, $bars, $vals, $dvalues, $dbars, $evalues, $ebars) 
#
#   Purpose:  Prints out the actual data for the triple vertical chart. 
#
#    Author: Jan Diepens
#
####################################################################### 
...
?>
         ...
         <IMG SRC="<? echo $bars[$i] ?>" WIDTH=6 HEIGHT="<? if ($values[$i]!=0){
		echo $values[$i] * $vals["scale"];
		} else { echo "1";} ?>">
         </TD><TD ALIGN="CENTER" VALIGN="BOTTOM"
<?
        ...
        if (! $vals["noshowvals"])
           {
            print '<I><FONT SIZE="-2" COLOR="' . $vals["triplefcolor"] . '" ';
            print ' STYLE="' . $vals["valuefstyle"] . '">'; 
            print $evalues[$i] . "</FONT></I><BR>";
           }
?>

         <IMG SRC="<? echo $ebars[$i] ?>" WIDTH=6 HEIGHT="<? if ($evalues[$i]!=0){
		echo $evalues[$i] * $vals["scale"];
		} else { echo "1";} ?>">
         </TD><TD ALIGN="CENTER" VALIGN="BOTTOM"
<?
...
?>
         ...
         <IMG SRC="<? echo $dbars[$i] ?>" WIDTH=6 HEIGHT="<? if ($dvalues[$i]!=0){
		echo $dvalues[$i] * $vals["scale"];
		} else { echo "1";} ?>">
         </TD></TR></TABLE>
	 </TD>
