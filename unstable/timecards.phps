...
if (mysql_num_rows($tcs)==0) {//no time cards...
	...
	if (('New TC'==$Action || ('Update TC'==$Action && mysql_result($MyTCs,0,'Status')=='Pending') && $NewDivider>mysql_result($MyTCs,0,'StopHidden')) && mysql_num_rows($FirstData)>0 && mysql_result($FirstData,0,0)>0) {
	...
	<TR><TD><B>Pending Regular:</B></TD><TD><?=$PendingReg?> Hrs</TR>
	<TR><TD><B>Pending Over Time:&nbsp;</B></TD><TD><?=$PendingOT?> Hrs</TR>
	</TABLE>
	<BR><BR>
