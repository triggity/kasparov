...
if(isset($_GET['data']) && $_GET['data'] == 'librarycount')
{
	...
	if(isset($_GET['type']) && $_GET['type'] == 'excelxml') //for when we support other formats
	{
		...
		{	
			...
			$exportdata[0][0] = '';
			$exportdata[0][1] = 'Basement PC';
			$exportdata[0][2] = 'Basement Mac';
			$exportdata[0][3] = '1st Floor PC';
			$exportdata[0][4] = '1st Floor Mac';
			$exportdata[0][5] = '2nd Floor 203';
			$exportdata[0][6] = '2nd Floor 205';
			$exportdata[0][7] = '2nd Floor 206';
			$exportdata[0][8] = '2nd Floor Mac';
			
			for ($t = 1; $t < 20; $t++)
			{
			        $curtime = ($t + 7) % 24 . ':00';
			        $exportdata[$t][0] = (($t + 7) % 12 == 0 ? '12' : ($t + 7) % 12) . ':00';
