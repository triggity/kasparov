...
if ($userdata["IsAdmin"]=="N" || ""==$IdNum) { $IdNum=$CampusID; }
	...
	$ThisCard=mysql_query("
		...
		WHERE
			...
			People.CampusID=$IdNum AND
			TimeCards_Members.CampusID=$IdNum AND
			TimeCards_Members.TimeCard=TimeCards_Periods.TimeCard
...
$ThisStart=-1;
$ThisEnd=-1;
$SuspiciousCount=0.0; //working while not scheduled
$DoubledCount=0.0; //hours working while everyone scheduled was also working.
for ($i=0;$i<mysql_num_rows($PinPoints);$i++) {
	if (mysql_result($PinPoints,$i,5)==0) { //not scheduled to work this time...
		$SuspiciousCount+=mysql_result($PinPoints,$i,6)*0.25;

/*
		//Who was supposed to be working?
		$PossablePeople=mysql_query("
...
*/
	...
	if (mysql_result($PinPoints,$i,0)!=$CurrentDay) {
...
//			echo "$ThisStart::$ThisEnd ".(floor($ThisStart*4)%4)."::".(floor($ThisEnd*4)%4)."<BR>\n";
			...
			$TableData[$row][$col+1]=floor($ThisEnd).":".(((floor($ThisEnd*60)%60)==0)?'00':(floor($ThisEnd*60)%60));
			$TableData[$row][9]+=$RowHours+($ThisEnd-$ThisStart);
			$TotalHours+=$RowHours+($ThisEnd-$ThisStart);
			$TableData[$row][10]='0';
			$TableData[$row][11]=$TableData[$row][9];//$TotalHours;
			if ($TableData[$row][9]!=$RowHours+($ThisEnd-$ThisStart)) {
				for ($l=0;$l<9;$l++)
					$TableData[15][$l]='  *OF*';
			}
		}
...
-0.008 Tc
0.021 Tw
[(POSITION #: <?=mysql_result($ThisCard,0,'ExtraInfo')?>)]TJ
T*
...
0.002 Tc
[()]TJ
[(Total:)-2106(<?=$TableData[16][11]?>)]TJ
T*
0.001 Tc
[(Suspect:)-1116(<?=$SuspiciousCount?>)-4696]TJ
0 9.12 -9.12 0 196.8 165.2 Tm
0.002 Tc
[(Doubled:)-537(.)]TJ
0 9.12 -9.12 0 207.6 79.2 Tm
0.002 Tc
[(OK:)-2825(.)]TJ
0 9.12 -9.12 0 207.6 165.2 Tm
0.002 Tc
[(Expected:)-246(.)]TJ
