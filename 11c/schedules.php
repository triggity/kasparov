...
*/
	...
	$cid=mysql_result($usernames,$us,1); //sore the campus ID for this user
		...
		if (0<$ClearDeltaCount || 0<$SetDeltaCount) {
			mysql_query("UPDATE Users SET lastschedchange=NOW() WHERE CampusID=$cid");
		}
		//Update the schedule
...
?><TABLE border=0 width=100%><TR><TD bgcolor=<?=$color_table_title?>><?=$ofont_title?>Calendar with Absences<?=$cfont_title?></TD></TR><TR><TD><?
calendar_view("today");
?></TD></TR></TABLE><?
...
<?
                ?><TD width=33% align=center><?
                        echo "<A HREF=\x22addabsence.php?Schedule=$Schedule&SID=$SID\x22>Post Absence</A>";
                ?></TD><TD width=33% align=center><?
        if (1==$notables) {
                echo "<A HREF=\x22schedules.php?Schedule=$Schedule&day=$oday&SID=$SID&printversion=$printversion&notables=0\x22>View Schedule WITH Title and Left Menu Bar</A></td>";
        } else {
                echo "<A HREF=\x22schedules.php?Schedule=$Schedule&day=$oday&SID=$SID&printversion=$printversion&notables=1\x22>View Schedule WITHOUT Title or Left Menu Bar</A></td>";
        }
	echo "<td align=center><A HREF=\x22schedules.php?Schedule=$Schedule&day=-1&SID=$SID&printversion=$printversion&notables=$notables\x22>View Weekly Schedule</A>";
        ?>
        </TD></TR>
	<?
	if ('Y'==$userdata["IsAdmin"] || 2<=$onthis) {
                ...
                if (1==$noedit) {
                        echo "<A HREF=\x22schedules.php?Schedule=$Schedule&day=$oday&SID=$SID&printversion=0&notables=$notables\x22>Edit Schedule</A>";
                } else {
                        echo "<A HREF=\x22schedules.php?Schedule=$Schedule&day=$oday&SID=$SID&printversion=1&notables=$notables\x22>View Schedule Times Only</A>";
                }
                ?></TD></TR><?
        }
