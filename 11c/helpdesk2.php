...
$times="";
$d=((int)date("G")<7?(date("w")+6)%7:date("w"));
$h=(int)date("H")*2 + (((int)date("i")<30)?0:1);
for ($i=0;$i<2;$i++) {
	if ($i>0) {$times.=" OR ";}
	$times.="Schedule.Time=".($d+(($h+$i)%48)*7);
}

MakeTable(mysql_query("
	SELECT
		".sql_nick("People.First","People.Middle","People.Last")." AS 'Name',
		".sql_phone("Phones.PhNum")." AS 'Phone'
	FROM Phones, Schedule, People
	WHERE	Phones.CampusID=People.CampusID AND
		People.CampusID=Schedule.CampusID AND
		Schedule.Position='RCC' AND
		(".$times.")
