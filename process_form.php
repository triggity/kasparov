...
if ((!is_numeric($count) || $count >= 99
      || $count != round($count)) && $_POST['submitted'] == 1)
{
	$count = $_POST[count];
...
if ((!is_numeric($late_count) ||
	  $late_count >= 99 || $late_count != round($late_count))
          && $_POST['late_submitted'] == 1)
{
...
if (!empty($late_count))
{
	$query = "INSERT INTO HelpCounts (Count, DATE) 
...
$trash_date = date("Y/m/d/h", strtotime("52 weeks ago"));
mysql_query("DELETE FROM HelpCounts WHERE DATE  <= '$trash_date'"); 
