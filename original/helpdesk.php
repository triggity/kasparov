...
<FORM>
<TR><TD bgcolor=<?=$color_table_title?> colspan=2><?=$ofont_title?>Look Up Person:<?=$cfont_title?></TD></TR>
<TR><TD BGCOLOR=<?=$color_table_lt_bg?> valign=top>
<TABLE border=0>
...
</TABLE></TD><TD BGCOLOR=<?=$color_table_lt_bg?> valign=top>
...
if ("Look Up"==$Action) {
	...
	<TR><TD BGCOLOR=<?=$color_table_title?>><?=$ofont_title?>Add Person to Database (only if
<I>NOT</I>
found by searching) (Alternate
Step 3)<?=$cfont_title?></TD></TR>
...
/* Call Tickets replaced by HEAT
MakeTable(mysql_query("SELECT t1.TicketID AS TicketID,concat('ticketstatus.php?TID=',t1.TicketID) AS TicketPopup, t1.Comment AS Description, IF((t2.Appointment > DATE_SUB(NOW(), INTERVAL 1 DAY)) AND (t2.Appointment < DATE_ADD(NOW(), INTERVAL 1 DAY)), concat('A',t2.Appointment), concat('B',t2.Creation)) AS Hidden, concat(IF(LENGTH(p.Nick)>0,concat(p.Nick,' ',p.Last),concat(p.First,' ',p.Last)),IF((0!=t2.Appointment),concat('<BR>',t2.Appointment),'')) AS 'Assignment', t2.Creation AS 'Last Update' ".(($userdata["IsIT"]=="Y" && $userdata["IsLINC"]=="Y")?", t2.Department AS Dept":"")." FROM PaperTrail as t1, PaperTrail as t2, People as p WHERE t1.TicketID=t2.TicketID AND t1.IsFirst='Y' AND t2.IsLast='Y' AND p.CampusID=t2.Receiver_CampusID AND t2.State='OPEN'".(($userdata["IsIT"]=="N")?" AND t2.Department='LINC'":(($userdata["IsLINC"]=="N")?" AND t2.Department='IT'":""))." ORDER BY Hidden"),1,1,1,3,"Open Call Tickets in your Department(s)");

?><BR><BR><?
*/

//Phone #s of RCCs on Call

$ourday=time()-18000; //Pretend it's 5hrs ago (to make the midnight crossing easier)
$ourtime=date("H",$ourday)*4+(int)(date("i",$ourday)/15); //figure out the point in the day from 5am in 15 minute increments

MakeTable(mysql_query("
        SELECT
                ".sql_nick("People.First","People.Nick","People.Last")." AS 'Name',
                ".sql_phone("Phones.PhNum")." AS 'Phone',
                Phones.Extra AS 'Extra Information'
        FROM Phones, Schedule_Data, People
        WHERE   Phones.CampusID=People.CampusID AND
                People.CampusID=Schedule_Data.CampusID AND
                Schedule_Data.Schedule=4 AND
                Schedule_Data.Day=".date("w",$ourday)." AND
                Schedule_Data.Time>=$ourtime AND
                Schedule_Data.Time<=".($ourtime+3)."
        GROUP BY Phones.PhNum

"),1,1,1,3,"Phone #s of Helpdesk Staff On Duty Now or in &lt; 1 hour");
...
MakeTable(mysql_query("
	...
	WHERE	Phones.CampusID=People.CampusID AND
		People.CampusID=Schedule_Data.CampusID AND
		Schedule_Data.Schedule=5 AND
		Schedule_Data.Day=".date("w",$ourday)." AND
		Schedule_Data.Time>=$ourtime AND
		Schedule_Data.Time<=".($ourtime+3)."
	GROUP BY Phones.PhNum

"),1,1,1,3,"Phone #s of RCCs On Duty Now or in &lt; 1 hour");

?><BR><BR><?

/* Call Tickets replaced by HEAT
MakeTable(mysql_query("SELECT t1.TicketID AS TicketID,concat('ticketstatus.php?TID=',t1.TicketID) AS TicketPopup, t1.Comment AS Description, IF((t2.Appointment > DATE_SUB(NOW(), INTERVAL 1 DAY)) AND (t2.Appointment < DATE_ADD(NOW(), INTERVAL 1 DAY)), concat('A',t2.Appointment), concat('B',t2.Creation)) AS Hidden, concat(IF(LENGTH(p.Nick)>0,concat(p.Nick,' ',p.Last),concat(p.First,' ',p.Last)),IF((0!=t2.Appointment),concat('<BR>',t2.Appointment),'')) AS 'Assignment', t2.Creation AS 'Last Update'  ".(($userdata["IsIT"]=="Y" && $userdata["IsLINC"]=="Y")?", t2.Department AS Dept":"")." FROM PaperTrail as t1, PaperTrail as t2, People as p WHERE t1.TicketID=t2.TicketID AND t1.IsFirst='Y' AND t2.IsLast='Y' AND p.CampusID=t2.Receiver_CampusID AND t2.State='ACTIVE'".(($userdata["IsIT"]=="N")?" AND t2.Department='LINC'":(($userdata["IsLINC"]=="N")?" AND t2.Department='IT'":""))." ORDER BY Hidden"),1,1,1,3,"ACTIVE Call Tickets in your Department(s)");
*/
