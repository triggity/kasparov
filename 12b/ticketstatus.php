...
<?
	...
	if ($IsLoggedIn==1 && ($userdata["IsHelpDesk"]=="Y" || $userdata["IsFieldSupport"]=="Y" || $userdata["IsAdmin"]=="Y")) { //Make sure User has permission to modify the ticket
		...
		if ("Update Jack"==$Action) { // 
		...
		} else if ("Update Client"==$Action && $IdNum>0 && $userdata["IsAdmin"]=="Y" && mysql_num_rows(mysql_query("SELECT CampusID FROM People WHERE CampusID=".(int)$IdNum))>0) {
			...
			$x=mysql_query("
				INSERT INTO PaperTrail (
					...
					$TID,
					NOW(),
					".(($Appt==1)?("'".date("Y-m-d H:i:00",mktime(((("PM"==$HalfDay && $Hour < 12) || ("AM"==$HalfDay && $Hour==12))?(($Hour+12)%24):$Hour),$Minute,0,$Month,$Day,$Year))."'"):"NULL")."
				)
			");
