...
<INPUT TYPE=HIDDEN NAME="Position" VALUE="<? echo $Position."\">";
	...
	?><TABLE border=0 width=100%><TR><TD bgcolor=<?=$color_table_title?>><?=$ofont_title?><? echo $Position; ?> Schedule for: <?
	switch ($day) {
		case 0:
			?>Sunday<?
			break;
...
<?
	if ('Y'==$userdata["IsAdmin"]) {
		...
		if (1==$noedit) {
			echo "<A HREF=\x22schedules.php?Position=$Position&day=$oday&SID=$SID&printversion=0&day=$oday&notables=$notables\x22>Edit Schedule</A>";
		} else {
			echo "<A HREF=\x22schedules.php?Position=$Position&day=$oday&SID=$SID&printversion=1&day=$oday&notables=$notables\x22>View Schedule Times Only</A>";
		}
